...
| 超速巡檢 | 需指定伺服器與旅團後執行 `!警報 開`；每 10 分鐘發一次結果，無超速者也會回報，完整區間去重避免重發 |
...
| DB 清理 | 從最近 1 次官方轉移窗到現在連續保留；近 **3** 天保留 10 分鐘快照，較舊橋接區同角同服只留首尾；另清過期轉服 log／`alert_dedupe`；大量刪除後建議離線 VACUUM |
