...
class ExpTracker(commands.Cog):
    ...
    async def investigate_player(self, ctx, target_name: str):
        ...
        if not p_row or not p_row[0]:
            return await processing_msg.edit(content=f"❌ 查無此人！資料庫中從未出現過 `{target_name}` (可能從未擠進過前50名)。")
        ...
        t_prev_row = self.cursor.fetchone()
        
        if not t_prev_row or not t_prev_row[0]:
            embed.add_field(name="⬅️ 前世身分", value=f"未發現跨服改名前科。\n*(可能自建檔以來即存在，或首度進入榜單)*\n初次建檔: `{p_min}`", inline=False)
        else:
            t_prev = t_prev_row[0]
            ...
            if best_match:
                ...
                embed.add_field(name="⬅️ 前世身分 (查獲跨服改名)", value=f"**{best_match['name']}** ({best_match['server']})\n誤差: {best_match['exp_diff']/1000000000000:+.2f} 兆 ({status})\n跳服時間: `{t_prev}`", inline=False)
            else:
                embed.add_field(name="⬅️ 前世身分", value=f"未發現跨服改名前科。\n*(可能自建檔以來即存在，或首度進入榜單)*\n初次建檔: `{p_min}`", inline=False)
        ...
        if p_max < db_max:
            ...
            t_next_row = self.cursor.fetchone()
            
            if not t_next_row or not t_next_row[0]:
                embed.add_field(name="➡️ 今生下落", value=f"去向不明。\n最後上線: `{p_max}`", inline=False)
            else:
                t_next = t_next_row[0]
                ...
                if best_match:
                    status = "📉掉趴" if best_match['exp_diff'] < -5000000000000 else ("📈贖回" if best_match['exp_diff'] > 50000000000000 else "⚖️平穩")
                    embed.add_field(name="➡️ 今生下落 (查獲跨服改名)", value=f"**{best_match['name']}** ({best_match['server']})\n誤差: {best_match['exp_diff']/1000000000000:+.2f} 兆 ({status})\n跳服時間: `{t_next}`", inline=False)
                else:
                    embed.add_field(name="➡️ 今生下落", value=f"去向不明。\n*(可能掉出前 50 名)*\n最後上線: `{p_max}`", inline=False)
        else:
